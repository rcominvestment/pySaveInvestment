...
def setData(name_table,l_date):
    ...
    setFirebase(df_finviz,name_table,l_date)

def setFirebase(df,l_date,l_data):

    sheet_name = "Data_Oportunity_Daily"
    ...
    data_5min = yf.download(l_TICKER, start=l_data, end=None, interval="1d", group_by='tickers')
...
def executed():
    ...
    l_date = str(year-1) + "-" + str(l_month) + "-" + str(l_day)
    
    connectFirebase()
    setData(name_table,l_date)
